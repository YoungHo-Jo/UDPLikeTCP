...
public class UDP {
	...
	static final int HEADER_SIZE = DATA_SEQUENCE_SIZE + ACK_SEQUENCE_SIZE 
												+ RESERVED_SIZE + FLAGS_SIZE + CHECKSUM_SIZE;
	...
	void makeHeaderAndSend(byte[] data) throws Exception {
		...
		printD("dSeq: " + byteArrayToHex(dataSequence) 
			+ " ACKSeq: " + byteArrayToHex(ACKSequence) 
			+ " reserved: " + byteArrayToHex(reserved) 
			+ " flags: " + byteArrayToHex(flags.toByteArray()) 
			+ " checksum: " + byteArrayToHex(checkSum));
	...
	byte[] receivePacket() throws Exception {
		receivingPacket = new DatagramPacket(new byte[MAXIMUM_SEGMENT_SIZE], 
	...
	void setCheckSum(byte[] target) {
		checkSum = ByteBuffer.allocate(CHECKSUM_SIZE)
				.putChar((char) getCheckSumValue(target)).array();
	}
	...
	boolean getFlag(Flag type, byte[] packetData) {
		byte[] flagsField = Arrays.copyOfRange(packetData, FLAGS_OFFSET, 
	...
	int getACKSequence(byte[] packetData) {
		byte[] ACKSeq = Arrays.copyOfRange(packetData, ACK_SEQUENCE_OFFSET, 
				ACK_SEQUENCE_OFFSET + ACK_SEQUENCE_SIZE);
